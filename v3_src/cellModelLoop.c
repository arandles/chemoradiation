...
void cellModelLoop(int run_id, int maxTimeSteps, double sens, int Z, int Zrevert, int ChemoWeeks, int ChemoDays, int ChemoDoses, int ThalfPerDose[7], int ChemoDay[7], int ChemoHour[7], double ChemoC[7], double Cmax, int RadWeeks, int radDoses, int radDose, int radDoseA[10], int radHour[10], int radDay[10], int *stemCount, int * tbCount, int *cellAge)
{
	...
	srand(run_id); /* initialize rand(), hardcoded for validation. Change to driven from the time step for full data collection. */	
	int VIS =0; //printf vis flag
	...
	for (cycle = 0; cycle < max_cycles; cycle++) {
				...
				if (cellDiffTime[i] <= 0) {
					if (cellQ[i] <= 0) {
						...
						if (hasExitedQ[i] == 0) {
							...
							double rate_per_hour = (cellType[i] == 0) ? rS : rTB;   // after setting cellType=1, this will use rTB
							double mean_div_hours = (rate_per_hour > 0.0) ? (1.0 / rate_per_hour) : 0.0;
		...
		for (T=1; T < maxTimeSteps; T++) {
			...
			for (i=0; i < cellCount; i++) {
				...
				if (cellDivTime[i] <= 0)  {
					if (cellQ[i] <= 0) {
						...
						int minDivSteps = (int) llround(((cellType[i] == 0) ? 12.0 : 24.0) * 60.0 * DT);
						cellDivTime[i] += minDivSteps;
						minDivSteps = (int) llround(((cellType[numCells] == 0) ? 12.0 : 24.0) * 60.0 * DT);
			...
			numKillCells = 0;
			giveRad = 0; 	
			static int rt_index = 0;   // counts RT fractions (not cells)
			...
			if (giveRad == 1) {
				...
				for (i=0; i < numCells; i++) {
					...
					if (sens_on && C > 0.0) {
						...
						sens_factor += 0.75 * s * recent_weight;
					}
			...
			/*****Handle Stem Cell Retention Near Vessel Boundary******/
			for (i=0; i < numCells; i++) {
				if (cellDist[i] < (Rvessel + RvesselCells + 3 * R)) cellType[i] = 0;
				else {
